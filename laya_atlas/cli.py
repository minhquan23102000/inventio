...
def cmd_sources(args) -> int:
    ...
    if not rows:
        print("no sources; run `laya-atlas init <path>`")
    return 0
...
def main(argv=None) -> int:
    ...
    p = argparse.ArgumentParser(prog="laya-atlas", description="Local retrieval map: BM25 + structure + links, reranked by Laya or TypeSafe.")
    p.add_argument("--version", action="version", version=__version__)
    p.add_argument("--db", help=f"map file (default {default_db()}, or $LAYA_ATLAS_DB)")
    ...
    def ranking(s):
        s.add_argument("--ranker", choices=RANKERS, default=os.environ.get("LAYA_ATLAS_RANKER", "none"),
